...
export function applyAccessibilityToCSS(accessibility: AccessibilityState): void {
  ...
  // コントラスト設定
  const contrastMultiplier = accessibility.contrast === 'high' ? 1.5 : 
                           accessibility.contrast === 'low' ? 0.7 : 1.0;
  ...
  // ゲーム固有設定
  root.style.setProperty('--grid-lines-opacity', accessibility.gameSpecific.gridLines ? '0.3' : '0');
  root.style.setProperty('--drop-preview-opacity', accessibility.gameSpecific.dropPreview ? '0.5' : '0');
}
 ...
 * 色覚異常対応色を取得
 */
export function getColorBlindnessSafeColor(originalColor: string, type: AccessibilityState['colorBlindnessType']): string {
  ...
  return colorMappings[type][originalColor as keyof typeof colorMappings[typeof type]] || originalColor;
}
...
export function generateAccessibilityReport(accessibility: AccessibilityState): {
  ...
  settings: {
    category: string;
    items: { name: string; value: string; status: 'optimal' | 'acceptable' | 'needs-improvement' }[];
  }[];
} {
