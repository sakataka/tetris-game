...
class AudioPreloader {
  ...
  public async preloadWithStrategy(
    ...
    const loadPromises = sortedSounds.map((soundPriority, index) => 
      this.preloadSingleSound(soundPriority, config, index * 100) // 100ms間隔で開始
    );
  ...
  private async preloadSingleSound(
    ...
    if (this.memoryUsage > config.memoryLimit * 1024 * 1024) {
      ...
      const error = new AudioError(
        ...
        { action: 'audio_preload', component: 'AudioPreloader', additionalData: { soundKey, memoryUsage: this.memoryUsage } },
      ...
      try {
        ...
        if (currentRetry > maxRetries) {
          ...
          const audioError = new AudioError(
            ...
            { action: 'audio_preload', component: 'AudioPreloader', additionalData: { soundKey, retries: currentRetry, error } },
            { recoverable: true, retryable: false }
          );
  ...
  public getProgress(): PreloadProgress {
    ...
    return {
      ...
      progress: statuses.length > 0 ? statuses.filter(s => s === 'loaded').length / statuses.length : 0
    };
  }
  ...
  public async preloadBasedOnNetwork(): Promise<PreloadProgress> {
    ...
    const connection = (navigator as unknown as { connection?: unknown; mozConnection?: unknown; webkitConnection?: unknown }).connection || (navigator as unknown as { mozConnection?: unknown }).mozConnection || (navigator as unknown as { webkitConnection?: unknown }).webkitConnection;
