...
describe('statisticsUtils', () => {
  ...
  describe('calculateSessionStatistics', () => {
    it('セッション統計を正しく計算する', () => {
      ...
      const longestSession = Math.max(...mockSessions.map(session => 
        session.endTime - session.startTime
      ));
      const totalGames = mockSessions.reduce((total, session) => 
        total + session.games.length, 0
      );
  ...
  describe('calculatePlayTimeStatistics', () => {
    it('プレイ時間統計を計算する', () => {
      // この関数は後で実装予定
      const sessionDurations = mockSessions.map(session => 
        session.endTime - session.startTime
      );
