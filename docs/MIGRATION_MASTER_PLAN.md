...
#### Week 3: 環境セットアップ
- **Day 1**: React Router 7プロジェクト初期化
- **Day 2**: Vite設定 + Tailwind CSS v4.1統合  
- **Day 3**: 基本ルート構造実装 (root.tsx, routes.ts)
- **Day 4**: shadcn/uiコンポーネント移行テスト
...
#### Week 7: パフォーマンス最適化
- **Day 1**: バンドル分析・コード分割最適化
- **Day 2**: React Router 7 SSR活用
- **Day 3**: Vite設定チューニング
- **Day 4**: メタデータ・SEO対応完全実装
- **Day 5**: アクセシビリティ機能検証
...
|------|------------------|------------------------|-------------|
| **ルーティング** | App Router | React Router 7 | Phase 2 |
| **フォント** | next/font/google | @fontsource/* | Phase 2 |
| **メタデータ** | Metadata API | Meta API | Phase 3 |
| **セキュリティヘッダー** | next.config.ts | helmet + Express | Phase 2 |
| **画像最適化** | Next.js Image | vite-plugin-imagemin | Phase 2 |
...
| 指標 | 現在 (Next.js) | 目標 (React Router 7) | 改善率 |
|------|----------------|----------------------|-------|
| **First Load JS** | 219 kB | ~200 kB | -8.7% |
| **Main Page** | 68.5 kB | ~65 kB | -5.1% |
| **HMR速度** | ~500ms | ~200ms | +150% |
| **ビルド時間** | ~1000ms | ~800ms | +25% |
| **開発サーバー起動** | ~2000ms | ~1000ms | +100% |
...
- [ ] 基本5ページ動作確認
- [ ] Vite + Tailwind v4.1動作確認
- [ ] shadcn/ui 15コンポーネント移行
- [ ] 開発ツール (Vitest, Biome) 動作
